...
namespace Realm {
  ...
  class ClearTimersMessage : public MessageType {
  protected:
	static ClearTimersMessage *m;

	ClearTimersMessage():MessageType(CLEAR_TIMER_MSGID, 0, false, true) { }
  ...
  class TimerDataRequestMessage : public MessageType {
  public:
	struct RequestArgs {
		void *rollup_ptr;
	};
	...
	virtual void request(Message *m);
	static void init();
	static void send(NodeId target, void *rollup_ptr);

  protected:
	TimerDataRequestMessage *m;

	TimerDataRequestMessage():MessageType(ROLL_UP_TIMER_MSGID, sizeof(RequestArgs), false, true) { }

  };
  
  class TimerDataResponseMessage : public MessageType {
  protected:
	TimerDataResponseMessage *m;

	TimerDataResponseMessage():MessageType(ROLL_UP_TIMER_RPLID, sizeof(RequestArgs), true, true) { }
  ...
  public:
	...
	virtual void request(Message *m);
	static void init();
	static void send(NodeId target, void *rollup_ptr, Payload *pl);
  };
}; // namespace Realm
