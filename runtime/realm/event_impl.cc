...
#define CHECK_EVENT(ev)					\
  ...
  struct MediumBroadcastHelper : public T::RequestArgs {
		   ...
		   int _payload_mode)
    {
      ...
      assert((payload_mode != FAB_PAYLOAD_FREE) && "cannot useFAB_PAYLOAD_FREE with broadcast!");
      targets.map(*this);
    }
  ...
  void EventUpdateMessageType::request(Message* m) {
    ...
    log_event.debug() << "event update: event=" << args->event
		      << " poisoned="
		      << ArrayOstreamHelper<EventImpl::gen_t>(new_poisoned_gens, new_poisoned_count);
...
#define CHECK_POISONED_GENS
  ...
  void GenEventImpl::trigger(gen_t gen_triggered, int trigger_node, bool poisoned)
  {
	...
	free_event = (num_poisoned_generations < POISONED_GENERATION_LIMIT);
      ...
      if(!to_update.empty())
	EventUpdateMessageType::broadcast_request(to_update, 
	...
	get_runtime()->local_event_free_list->free_entry(this);
      ...
      EventImpl::gen_t previous_subscribe_gen = 0;
				   ...
				   const void *reduce_value, size_t reduce_value_size)
  {
	...
	if(local_notifications.empty() && (remote_notifications.size() == 1) &&
	   (ID(me).barrier.creator_node == fabric->get_id())) {
  ...
  bool BarrierImpl::has_triggered(gen_t needed_gen, bool& poisoned)
  {
	...
	BarrierSubscribeMessageType::send_request(owner, me.id, needed_gen,
						  fabric->get_id(), false/*!forwarded*/);
      }
    }
  ...
  void BarrierTriggerMessageType::send_request(NodeId target, ID::IDType barrier_id,
    ...
    FabContiguousPayload* payload = new FabContiguousPayload(FAB_PAYLOAD_COPY, (void*) data, datalen);
   
    fabric->send(new BarrierTriggerMessage(target, fabric->get_id(), barrier_id, trigger_gen, previous_gen,
