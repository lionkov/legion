	...
	*msgptr++ = oasvec.size();
	...
	for(OASByInst::iterator it = oas_by_inst->begin(); it != oas_by_inst->end(); it++) {
...

	  {
	    ...
	    if(!e.has_triggered()) {
	      if(just_check) {
		log_dma.debug("dma request %p - no dst instance (" IDFMT ") metadata yet",
			      this, dst_impl->me.id);
		return false;
	      }
	      log_dma.debug() << "request " << (void *)this
			      << " - dst instance metadata invalid - sleeping on event " << e;
	      waiter.sleep_on_event(e);
	      return false;
	    }
	  }
...
namespace Realm {
	  ...
	  if (oas.serdez_id != 0) {
	    ...
	    CopyRequest *r = new CopyRequest(*this, oas_by_inst,
	      ...
	      RemoteCopyMessageType::RequestArgs* args = new RemoteCopyMessageType::RequestArgs;
              ...
              log_dma.debug("performing serdez on remote node (%d), event=" IDFMT, dma_node,
			    args->after_copy.id);
	      ...
	      fabric->send(new RemoteCopyMessage(dma_node, (void*) args, payload));
	...
	for(OASByMem::const_iterator it = oas_by_mem.begin(); it != oas_by_mem.end(); it++) {
	  ...
	  if(((unsigned)dma_node) == fabric->get_id()) {
	    ...
	    log_dma.debug("performing copy on remote node (%d), event=" IDFMT, dma_node,
			  args->after_copy.id);
	    get_runtime()->optable.add_remote_operation(ev, dma_node);
